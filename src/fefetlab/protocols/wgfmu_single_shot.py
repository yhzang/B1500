...
# 单写三协议(E1S/E6S/E6M)· 脆弱 L10 pFeFET 专用,每 shot 只写一次。
# 2026-06-23 从 scripts/wgfmu_single_shot_disturb.py 整体搬入本包(脚本退化为薄壳),
# 以便升格进 engine REGISTRY 让 GUI 可驱动;协议逻辑零改写,仍复用 wgfmu_fefet 的
# backend / orchestration / 读窗积木。CLI 行为(stdout/CSV/manifest)字节级不变。
from . import wgfmu_fefet as base  # reuse backend + orchestration primitives
...
def run_stage_e6m(backend, args, *, callbacks=None):
...
def run_stage_e1s(backend, args, *, callbacks=None):
...
# rest_anchor 落仓库根 runs/(base.ROOT = 仓库根,与脚本时代 _SRC.parent 同一目录)。
REST_ANCHOR_PATH = base.ROOT / "runs" / "rest_anchor.txt"
