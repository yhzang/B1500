...
摘要参数表 + 栅波形阶梯图(标出读窗)。只读,关掉即走。

波形来自协议真实 build(经 AuditBackend 记录),= live 时会下发的同一条。
"""
...
class TimingPreviewDialog(QDialog):
    def __init__(self, preview: dict, parent=None) -> None:
        ...
        try:
            ...
            pw.setLabel("left", "Gate V")
            pw.showGrid(x=True, y=True, alpha=0.3)
            gx: list[float] = []
            gy: list[float] = []
            for t0, t1, v in preview.get("gate_timeline", []):
                gx += [t0, t1]
                gy += [v, v]
            if gx:
                pw.plot(gx, gy, pen=pg.mkPen("#378ADD", width=2))
            for te in preview.get("read_events_s", []):
                pw.addItem(pg.InfiniteLine(pos=te, angle=90,
                                           pen=pg.mkPen("#D85A30", style=Qt.PenStyle.DashLine)))
            lay.addWidget(pw, 1)
            note = QLabel("蓝线 = 栅电压波形;橙色虚线 = 读窗。代表段(E6M 取一个 checkpoint 读段);时序由 WGFMU 5ns 硬件时基播放。")
